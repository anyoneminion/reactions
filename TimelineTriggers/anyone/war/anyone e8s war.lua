...
\
\
if SallyWAR.SkillSettings.Potion.enabled == true then\
data.PotsEnabled = true\
end\
\
\
		if Player.job ~= 21 and AnyoneReactionSettings.Settings.JobCheck == true then\
				d(\"[Anyone's Reactions] - Job check failed, sending text command.\")\
				SendTextCommand(\"/e You're using the wrong timeline triggers. Check that your timeline triggers is set to the warrior profile.\")\
		elseif Player.job == 21 then\
				d(\"[Anyone's Reactions] - Player job check succeeded\")\
		elseif Player.job ~= 21 and AnyoneReactionSettings.Settings.JobCheck == false then\
